...
################################################################
def load_NTUdataset(datadir='NTU60',mode='train',evaluation='CS'):
    ############################################
    rootdir = '/home/yhx/media/msx'
    ...
    return dsets 
############################################
def load_UWA3D(datadir='/dev/Data/NUCLA/',mode='train'):
    ############################################
    fread = h5py.File(datadir + mode + '.h5py','r')
    x =  fread['data'][:] 
    y =  fread['labels'][:] 
    fread.close()
    ############################################x
...
def img2seq(img):
    ...
    return seq
  #########################################################################################################  
def seq2img(seq,joint=50):
    img = seq.reshape(seq.size(0),seq.size(1),-1,joint).permute(0,2,1,3) if len(seq.size())==3 else seq
    return img
########################################################################################## 
